...
{
    public class XML_NotaCreditoExport
    {
        public putCustomerETDLoad Generar(int DocEntry)
        {
            ...
            query = @"Select ""Indicator"", ""DocNum"", ""DocDate"", ""TaxDate"", ""DocTotal"", ""DocTotalFC"", ""DocCur"",
                            ...
                            ""U_FE_FPEXP"", ""U_FE_MVEXP"", ""U_FE_CVEXP"", ""U_FE_VTRANSP"", ""U_FE_PTOEMB"",
                           ""U_FE_PTODESEMB"", ""U_FE_PAIS"", ""U_FE_TBULTO"", ""U_FE_TOTBULTO"",
            ...
            if (!oRec.EoF)
            {
                ...
                emision.Encabezado.camposEncabezado.TipoDTE = "112";
                ...
                query = @"Select top 1 * from ""RIN1"" where ""DocEntry"" = " + DocEntry + "";
                oRecG.DoQuery(query);
                ...
                emision.Encabezado.camposEncabezado.FmaPagExp = oRec.Fields.Item("U_FE_FPEXP").Value.ToString();
                emision.Encabezado.camposEncabezado.CodModVenta = oRec.Fields.Item("U_FE_MVEXP").Value.ToString();
                emision.Encabezado.camposEncabezado.CodClauVenta = oRec.Fields.Item("U_FE_CVEXP").Value.ToString();
                emision.Encabezado.camposEncabezado.CodViaTransp = oRec.Fields.Item("U_FE_VTRANSP").Value.ToString();
                emision.Encabezado.camposEncabezado.CodPtoEmbarque = oRec.Fields.Item("U_FE_PTOEMB").Value.ToString();
                emision.Encabezado.camposEncabezado.CodPtoDesemb = oRec.Fields.Item("U_FE_PTODESEMB").Value.ToString();
                emision.Encabezado.camposEncabezado.CodPaisRecep = oRec.Fields.Item("U_FE_PAIS").Value.ToString();
                emision.Encabezado.camposEncabezado.CodPaisDestin = oRec.Fields.Item("U_FE_PAIS").Value.ToString();
                ...
                // Bultos
                TipoBultosc bultos = new DBNet.TipoBultosc();
                bultos.CodTpoBultos = oRec.Fields.Item("U_FE_TBULTO").Value.ToString();
                bultos.CantBultos = oRec.Fields.Item("U_FE_TOTBULTO").Value.ToString();
                ...
                if (impMarca.Equals("SI"))
                {
                    query = @"Select
                            ...
                            ""RIN1"".""Price"" as ""Price"",
                            ""RIN1"".""DiscPrcnt"" as ""DiscPrcnt"",
                            (""RIN1"".""DiscPrcnt"" * ""RIN1"".""PriceBefDi"" * ""RIN1"".""Quantity"" /100) as ""MtoDescuento"",
                            ""RIN1"".""TotalFrgn"" as ""TotalFrgn"",
                            ""RIN1"".""PackQty"" as ""PackQty"",
                            ""RIN1"".""SubCatNum"" as ""SubCatNum"",
                ...
                else
                {
                    query = @"Select *, (""DiscPrcnt"" * ""PriceBefDi"" * ""Quantity"" /100) as ""MtoDescuento"" 
                            from ""RIN1"" where ""DocEntry"" = " + DocEntry + "";
                }
                oRecG.DoQuery(query);
                int count = 1;
                while (!oRecG.EoF)
                ...
                if (!string.IsNullOrEmpty(oRec.Fields.Item("U_TDBSII").Value.ToString()))
                {
                    ...
                    referencias.CodRef = oRec.Fields.Item("U_CODREFDB").Value.ToString();
                    emision.DescuentosRecargosyOtros.Referencias.Referencias.Add(referencias);
                }
