    ...
    public class XML_NotaCredito
    {
        public putCustomerETDLoad Generar(int DocEntry)
        {
            ...
            if (!oRec.EoF)
            {
                ...
                emision.Encabezado.camposEncabezado.TipoDTE = "61";
                ...
                oRecG.DoQuery(query);
                string impCatalogoSN = oRecG.Fields.Item("U_IMPCATALOGO").Value.ToString();
                string impCodigoSN = oRecG.Fields.Item("U_IMPCODIGO2").Value.ToString();
                string impDescriSN = oRecG.Fields.Item("U_IMPDESCRIPCION2").Value.ToString();
                ...
                bool Resumida = oRec.Fields.Item("U_TIPOFACTURA").Value.ToString().Equals("1") ? true : false;
                if (Resumida)
                {
                    int countguias = 0;
                    // Referencias Facturas
                    string glosaFacturas = "SEGUN FACTURA N° ";
                    query = @"Select distinct(""BaseEntry"") 
                    from ""RIN1"" 
                    where ""BaseType"" = 13 and ""DocEntry"" = " + DocEntry + "";
                    oRecG.DoQuery(query);
                    while (!oRecG.EoF)
                    {
                    ...
                    where ""DocEntry"" = " + oRecG.Fields.Item("BaseEntry").Value.ToString() + "";
                        ...
                        while (!oRecG2.EoF)
                        {
                            ...
                            {
                                glosaFacturas += oRecG2.Fields.Item("FolioNum").Value.ToString();
                            }
                            else
                            {
                                glosaFacturas += " - " + oRecG2.Fields.Item("FolioNum").Value.ToString();
                    ...
                    if (countguias > 0)
                    {
                        ...
                        detalle.Detalles.NroLinDet = "1";
                        detalle.Detalles.NmbItem = "FE";
                        detalle.Detalles.DscItem = glosaFacturas.ToString();
                        detalle.Detalles.QtyItem = "1";
                        detalle.Detalles.PrcItem = oRec.Fields.Item("Neto").Value.ToString().Replace(",", ".");
                        detalle.Detalles.MontoItem = oRec.Fields.Item("Neto").Value.ToString().Replace(",", ".");
                        detalle.Detalles.QtyRef = "1";
                ...
                else
                {
                    ...
                    while (!oRecG.EoF)
                    {
                        ...
                        query = @"Select ""Substitute"", ""U_FE_CODIGO2"", ""U_FE_DESCRIPCION2"" 
                        from ""OSCN""
                        ...
                        if (impCodigoSN.Equals("Y"))
                        {
                            ...
                            codItems.CodItem = oRecG2.Fields.Item("U_FE_CODIGO2").Value.ToString().PadLeft(35, ' ').Substring(0, 35).Trim();
                            //codItemsb = new CodItemsb();
                            codItemsb.CodItems.Add(codItems);
                            //detalle.CodItems.Add(codItemsb);
                        }
                        if (impDescriSN.Equals("Y"))
                        {
                            ...
                            codItems.CodItem = oRecG2.Fields.Item("U_FE_DESCRIPCION2").Value.ToString().PadLeft(35, ' ').Substring(0, 35).Trim();
                            //codItemsb = new CodItemsb();
                            codItemsb.CodItems.Add(codItems);
                            //detalle.CodItems.Add(codItemsb);
                        }
