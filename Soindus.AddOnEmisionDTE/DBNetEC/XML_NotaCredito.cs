    ...
    public class XML_NotaCredito
    {
        public putCustomerETDLoad Generar(int DocEntry)
        {
            ...
            query = @"Select ""Indicator"", ""DocNum"", ""DocDate"", ""TaxDate"", ""DocTotal"",
                            ...
                            ""U_VK_ReferenciaFC"", ""U_VK_TipoReferenciaFC"", ""U_VK_FechaRefFC"", ""U_VK_CodReferencia"",
            ...
            if (!oRec.EoF)
            {
                ...
                if (!string.IsNullOrEmpty(oRec.Fields.Item("U_VK_ReferenciaFC").Value.ToString()))
                {
                    countref++;
                    referencias = new DBNetEC.Referencias();
                    referencias.NroLinRef = countref.ToString();
                    referencias.TpoDocRef = oRec.Fields.Item("U_VK_TipoReferenciaFC").Value.ToString();
                    referencias.FolioRef = oRec.Fields.Item("U_VK_ReferenciaFC").Value.ToString();
                    referencias.FchRef = DateTime.Parse(oRec.Fields.Item("U_VK_FechaRefFC").Value.ToString()).ToString("yyyy-MM-dd");
                    referencias.CodRef = oRec.Fields.Item("U_VK_CodReferencia").Value.ToString();
                    emision.DescuentosRecargosyOtros.Referencias.Referencias.Add(referencias);
                }
