...
namespace Soindus.AddOnEmisionDTE.Formularios
{
    ...
    class F133 : SystemFormBase
    {
            ...
            try
            {
                if (BusinessObjectInfo.BeforeAction == false)
                {
                    if (BusinessObjectInfo.ActionSuccess == true)
                    {
                        ...
                        //    oButton = (SAPbouiCOM.Button)oForm.Items.Item("btnDTE").Specific;
                        //    oButton.Item.Visible = true;
                        //    oButton = (SAPbouiCOM.Button)oForm.Items.Item("btnPDF").Specific;
                        //    oButton.Item.Visible = true;
                        //}
                        //else
                        //{
            ...
            if (pVal.EventType != SAPbouiCOM.BoEventTypes.et_FORM_UNLOAD && pVal.Before_Action == true)
            {
                ...
                if (pVal.EventType == SAPbouiCOM.BoEventTypes.et_CLICK && pVal.ItemUID.Equals("btnPDF") && pVal.Before_Action == true)
                {
                    if (pVal.FormMode == (int)SAPbouiCOM.BoFormMode.fm_OK_MODE)
                    {
                        ...
                        {
                            ObtenerPDF(oItem.Value, "33", "FE", "false");
                        }
                        else
                        {
                ...
                //    oNewItem = oForm.Items.Add("btnXML", SAPbouiCOM.BoFormItemTypes.it_BUTTON);
                ...
                //    SAPbouiCOM.Button oButton;
                //    oButton = ((SAPbouiCOM.Button)(oNewItem.Specific));
                //    oButton.Caption = "Generar XML";
                //}

                //if (pVal.EventType == SAPbouiCOM.BoEventTypes.et_CLICK && pVal.ItemUID.Equals("btnXML") && pVal.Before_Action == true)
                //{
                //    Application.SBO_Application.MessageBox("generando XML");

                //    SAPbouiCOM.EditText oItem;
                //    oItem = (SAPbouiCOM.EditText)oForm.Items.Item("8").Specific;
                //    ProveedorDTE proveedorDTE = new ProveedorDTE();
                //    string[] parametros = new string[] { oItem.Value, "33" };
                //    var result = proveedorDTE.CrearDocumento(parametros);
                //    if (result.Success)
                //    {
                //        var resp = proveedorDTE.Respuesta;
                //        // Guardar Folio
                //        parametros = new string[] { "", resp.Folio, "33", "", "", "false" };
                //        result = proveedorDTE.ObtenerPDF(parametros);
                //        if (result.Success)
                //        {
                //            //Guardar PDF
        ...
        public static void ObtenerPDF(string folio, string tipodoc, string folioprefix, string cedible)
        {
            ...
            {
                parametros = new string[] { "", folio, tipodoc, oDoc.DocTotal.ToString(), oDoc.DocDate.ToString("yyyy-MM-dd"), cedible };
            }
